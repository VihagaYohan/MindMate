...
const MoodEntry = ({ navigation }: PropTypes) => {
    ...
    // Helper function to draw face
    const drawFace = () => {
        return (
            <Canvas style={{ flex: 1 }}>
                <Circle cx={circleRadius} cy={circleRadius} r={circleRadius} color="cyan" />
            </Canvas>
        )
    }
    ...
    return (
        <AppContainer>
            <AppText text='How are you feeling today ?' textStyle={styles(isDarkMode).title} fontSize={14} />

            {/* canvas drawing area */}
            {/*             <Animated.View
                style={[styles(isDarkMode).canvasContainer,
                    backgroundStyle
                ]}
                onLayout={(e) => {
            ...
            <View style={[styles(isDarkMode).canvasContainer]}>
                <Canvas style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                    <Circle cx={circleRadius} cy={circleRadius} r={circleRadius} color="yellow" />
                </Canvas>
            </View>
